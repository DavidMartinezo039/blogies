<div class="mx-auto mt-4 max-w-6xl">
    ...
    <form action="{{ route('posts.index') }}" method="GET" class="mb-4 flex justify-center space-x-4">
        ...
        <div class="flex space-x-4 items-center">
            ...
            <div class="relative inline-block">
                ...
                <select name="order_by" id="order_by"
                        class="p-2 rounded border border-gray-300 bg-white shadow-sm focus:border-blue-400 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                    <option value="published_at" {{ $orderBy == 'published_at' ? 'selected' : '' }}>Fecha de
                        publicación
                    </option>
                    <option value="created_at" {{ $orderBy == 'created_at' ? 'selected' : '' }}>Fecha de creación
                    </option>
                </select>
            </div>
        ...
        <!-- Botón de aplicar -->
        <button type="submit"
                class="text-lg font-semibold group flex items-center rounded-full bg-sky-600 px-3 py-2 text-sky-100 shadow-lg duration-300 hover:bg-sky-700 active:bg-sky-800">
            Ordenar
        </button>
    </form>
